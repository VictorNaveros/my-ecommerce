...
// Variables globales
const CART_KEY = 'cart'; // 👈 clave unificada con main.js
...
window.addToCart = function(product) {
    ...
    if (existing) {
        ...
        cartItems.push({
            ...
            name: product.name,
            price: product.price,
            image: product.image || '../assets/images/prueba.jpg',
            quantity: 1
        });
...
function createCartItemHTML(item) {
    return `
        <div class="bg-gradient-to-tr from-black via-gray-900 to-red-950 rounded-xl shadow-lg p-6 cart-item" data-id="${item.id}">
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                ...
                <div class="flex-grow min-w-0">
                    ...
                    <p class="text-gray-400">${formatPrice(item.price)} cada uno</p>
                    <p class="text-sm text-gray-400 mt-1">Total: ${formatPrice(item.price * item.quantity)}</p>
                </div>
                
                <!-- Controles de cantidad -->
                <div class="flex items-center gap-3">
                    <button onclick="updateQuantity('${item.id}', ${item.quantity - 1})" 
                            class="w-8 h-8 rounded-full bg-gray-600 hover:bg-gray-300 flex items-center justify-center transition-colors duration-200" 
